...
yes_button.addEventListener('click', () => {
    ...
    heartButton.addEventListener('click', () => {
        Swal.fire({ 
            text: "I love U Makๆๆๆ T sud nai lokkk eiei \n ❤️❤️❤️❤️❤️",
    ...
    document.getElementById("heart-button").addEventListener("click", function() {
        ...
        if (audio.paused) {
            audio.play(); // Play the song if it's paused
        } else {
            audio.pause(); // Pause the song if it's already playing
            audio.currentTime = 0; // Restart the song when clicked again
        }
    });
